...
export async function sendPolicyUpdateEmail(
  ...
  if (!transporter) {
    console.warn(
      "[email] SMTP not configured — skipping policy update email."
    );
    return;
  }
  ...
  const subject =
    type === "terms"
      ? "Inlook Terms of Service update"
      : type === "privacy"
      ? "Inlook Privacy Policy update"
      : "Inlook Terms & Privacy Policy update";

  const trimmedName = (name ?? "").trim();
  const greeting = trimmedName ? `Hi ${esc(trimmedName)},` : "Hi,";

  const links = (() => {
    if (type === "terms") {
      return `<a href="${baseUrl}/terms" style="color:${BRAND.accent};text-decoration:underline">View the updated Terms of Service</a>`;
    }
    if (type === "privacy") {
      return `<a href="${baseUrl}/privacy" style="color:${BRAND.accent};text-decoration:underline">View the updated Privacy Policy</a>`;
    }
    return `<a href="${baseUrl}/terms" style="color:${BRAND.accent};text-decoration:underline">Updated Terms of Service</a> &middot; <a href="${baseUrl}/privacy" style="color:${BRAND.accent};text-decoration:underline">Updated Privacy Policy</a>`;
  })();
  ...
  const html = layout(`
    ...
    <p style="margin:0 0 20px;font-size:15px;line-height:1.7;color:${BRAND.text}">
      We're writing to let you know that our <strong>${typeLabel}</strong> has been updated.
    </p>
    ${
      summary
        ? `<div style="margin:0 0 20px;padding:16px 18px;border:1px solid ${BRAND.border};border-radius:12px;background:${BRAND.bg}">
             <p style="margin:0 0 6px;font-size:11px;letter-spacing:0.14em;text-transform:uppercase;color:${BRAND.muted}">Summary of changes</p>
             <p style="margin:0;font-size:14px;line-height:1.6;color:${BRAND.text}">${esc(summary)}</p>
           </div>`
        : ""
    }
    <p style="margin:0 0 20px;font-size:15px;line-height:1.7;color:${BRAND.text}">
    ...
    <p style="margin:0 0 20px;padding:14px 16px;border-left:3px solid ${BRAND.accent};background:${BRAND.bg};font-size:14px;line-height:1.7;color:${BRAND.muted};font-style:italic">
      &ldquo;Your continued use of the Service after the effective date means you accept the updated Terms.&rdquo; <span style="color:${BRAND.muted};font-style:normal">(Sect. 17, Terms of Service)</span>
    </p>
    <p style="margin:0 0 20px;font-size:14px;line-height:1.7;color:${BRAND.muted}">
      ...
      <a href="mailto:support@inlookdeals.com" style="color:${BRAND.accent}">support@inlookdeals.com</a>.
    </p>
    <p style="margin:0;font-size:15px;line-height:1.7;color:${BRAND.text}">
      &mdash; The Inlook team
    </p>
  `);
