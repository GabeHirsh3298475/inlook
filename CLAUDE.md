...
**`tiktok_url` population:** `fetchUserInfo` requests `profile_deep_link` (enabled by the `user.info.profile` scope) and we persist it as `tiktok_url` on the creator row during both `POST /api/apply` and `POST /api/creator/refresh-tiktok-stats`. This is the URL the "TikTok · Followers" badge on the network card + full profile links to. Creators who connected before `user.info.profile` was enabled will have `tiktok_url = null` until they hit Refresh on the dashboard or re-auth via `/apply`.
...
- **Per-platform follower boxes:** below the Total Followers number on both the card and the full-profile Basic Information block, one bordered box per connected platform shows the platform icon + follower count + role label ("Subscribers" for YouTube, "Followers" for TikTok/Instagram), wrapped in an `<a>` to the creator's platform URL. For TikTok the URL comes from `tiktok_url` (i.e. `profile_deep_link`); for YouTube from `channel_url`; for Instagram from the legacy `instagram_url` field.
...
- Clickwrap-style consent is enforced at three entry points: creator apply (the "Connect at least one account" block in `app/apply/apply-client.tsx` — "By clicking connect, you agree to our privacy policy and terms of service" with links to `/privacy` and `/terms`, sits directly above the YouTube/TikTok Connect buttons), brand apply (`Request access` button, in `components/brand-application-form.tsx`), and waitlist (`Join the waitlist` button, in `app/waitlist/waitlist-client.tsx`).
...
- Both policies carry **Effective date: April 22, 2026 · Last updated: April 23, 2026**. When making any material change, bump Last-updated in both [app/terms/page.tsx](app/terms/page.tsx) and [app/privacy/page.tsx](app/privacy/page.tsx).
- **TikTok coverage in policies (for TikTok Login Kit review):** Privacy §2 has a dedicated TikTok data paragraph (scopes, stored fields); Privacy §5 is titled "Platform API services and how to revoke access" and contains a two-step TikTok revoke procedure — (1) revoke on TikTok's side at `tiktok.com/setting/connected-apps` (mobile path: Settings and privacy → Security and permissions → Apps and services permissions → Inlook → Remove access), (2) email `support@inlookdeals.com` to purge stored tokens + cached TikTok fields (7-day active deletion, 90-day backup purge). TOS §4 renamed to "Platform integrations (YouTube and TikTok)" and mirrors the scopes + revoke path. When adjusting TikTok scopes in [lib/tiktok.ts](lib/tiktok.ts), update the scope list in both legal pages and in §1 of this doc.
